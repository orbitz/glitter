...
let predict training_fname fasta_fname start_end train viterbi create_training_data ng sg =
  ...
  let td = Seq.to_list (create_training_data gene_boundaries fasta) in
  let training_data = Seq.map (fun (s, v) -> (s, Misc_lib.list_of_string v)) (Seq.of_list td) in
  let hmm = train start_end training_data in
  let (total, path, prob) = viterbi (Fasta.to_seq (Fasta.read_file ~chunk_size:10000 fasta_fname)) hmm in
  ...
  ((total, path, prob), gene_list, count, training_count)
